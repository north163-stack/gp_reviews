...
# 1. 页面级配置 & 全局 CSS
# ==========================================
st.set_page_config(page_title="Bubble Shooter AI 智能体检", page_icon="📱", layout="wide")

st.markdown("""
<style>
    .report-header { font-size: 2.8em; font-weight: 800; color: #1f77b4; border-bottom: 3px solid #f0f2f6; padding-bottom: 10px; margin-bottom: 20px;}
    .section-title { font-size: 1.8em; font-weight: 700; color: #2c3e50; margin-top: 40px; margin-bottom: 20px; border-left: 8px solid #1f77b4; padding-left: 15px;}
    div[data-testid="metric-container"] { background-color: #f8f9fa; border-radius: 10px; padding: 20px; border: 1px solid #e9ecef; box-shadow: 2px 2px 5px rgba(0,0,0,0.05);}
</style>
...
def get_zeus_style_insight(model, df):
    ...
    prompt = f"""作为传音高级游戏数据分析师，请阅读以下真实玩家评论，生成结构化诊断报告。
    要求包含：执行摘要、关键发现(引用原话)、核心用户画像、及针对研发/策划/发行的短中长期建议。
    数据源：{chr(10).join(sample_reviews)}"""
    try:
...
def get_issue_categorization(model, dataframe):
    neg_samples = dataframe[dataframe['score'] <= 2]
    if neg_samples.empty: return None
    samples = neg_samples.sample(min(25, len(neg_samples)))['content'].tolist()
    prompt = f"请将以下游戏评论归类为：广告体验、关卡难度、性能问题、商业化、核心玩法。格式：分类|评论。数据：{samples}"
    try:
        res = model.generate_content(prompt)
        data = [line.split('|', 1) for line in res.text.strip().split('\n') if '|' in line]
        return pd.DataFrame(data, columns=['Category', 'Comment'])
    except: return None
...
with st.sidebar:
    ...
    if os.path.exists(DATA_FILENAME):
        st.success(f"数据源已就绪: {DATA_FILENAME}")
    else:
        st.error("🚨 缺失数据文件！")
    
    if gemini_model:
        st.success("AI 引擎：Gemini 已连接")
    else:
        st.warning("AI 引擎：离线模式 (无法生成报告)")
...
if os.path.exists(DATA_FILENAME):
    ...
    if df is not None:
        ...
        col1.metric("累计分析样本", f"{len(df):,}")
        col2.metric("平均星级评分", f"{avg_score:.2f} ⭐")
        col3.metric("NLP 情感均分", f"{df['nlp_sentiment_score'].mean():.2f}")
        col4.metric("真实负面口碑", f"{neg_pct:.1f}%", delta_color="inverse")

        # --- 模块 2: 版本迭代质量分析 (新增炫酷模块) ---
        st.markdown('<div class="section-title">🚀 版本迭代质量分析</div>', unsafe_allow_html=True)
        v_stats = df.groupby('reviewCreatedVersion').agg({'score': 'mean', 'at': 'count'}).rename(columns={'at': 'Count'}).reset_index()
        v_stats = v_stats[v_stats['Count'] > 5].sort_values('reviewCreatedVersion', ascending=False)
        
        if not v_stats.empty:
            v_col1, v_col2 = st.columns([3, 2])
            with v_col1:
                fig_v = make_subplots(specs=[[{"secondary_y": True}]])
                fig_v.add_trace(go.Scatter(x=v_stats['reviewCreatedVersion'], y=v_stats['score'], name="平均评分", mode='lines+markers', line=dict(color='#1f77b4', width=4)), secondary_y=False)
                fig_v.add_trace(go.Bar(x=v_stats['reviewCreatedVersion'], y=v_stats['Count'], name="评价基数", opacity=0.2, marker_color='gray'), secondary_y=True)
                fig_v.update_layout(height=400, margin=dict(l=20, r=20, t=20, b=20), hovermode="x unified")
                st.plotly_chart(fig_v, use_container_width=True)
            with v_col2:
                if len(v_stats) >= 2:
                    diff = v_stats.iloc[0]['score'] - v_stats.iloc[1]['score']
                    st.metric(f"最新版本 {v_stats.iloc[0]['reviewCreatedVersion']}", f"{v_stats.iloc[0]['score']:.2f} ⭐", delta=f"{diff:.2f}")
                    st.info("💡 **诊断建议**：若评分持续下滑，请立即对照下方‘业务痛点下钻’模块定位具体原因。")

        # --- 模块 3: AI 深度洞察 ---
        st.markdown('<div class="section-title">🤖 AI 专家级深度洞察</div>', unsafe_allow_html=True)
        if st.button("⚡ 生成专家级分析报告", type="primary"):
            if gemini_model:
                with st.spinner('AI 正在交叉验证数据...'):
                    report = get_zeus_style_insight(gemini_model, df)
                    st.markdown(f"<div style='background-color: #f1f3f6; padding: 25px; border-radius: 10px; border-left: 5px solid #1f77b4; color: #2c3e50;'>{report}</div>", unsafe_allow_html=True)
            else: st.error("请先配置 API Key")

        # --- 模块 4: 业务下钻热力图 (新增炫酷模块) ---
        st.markdown('<div class="section-title">🎯 业务痛点下钻 (AI 分类)</div>', unsafe_allow_html=True)
        if st.button("🔍 开启 AI 自动化归类"):
            with st.spinner("分类中..."):
                issue_df = get_issue_categorization(gemini_model, df)
                if issue_df is not None:
                    cat_counts = issue_df['Category'].value_counts().reset_index()
                    cat_counts.columns = ['Category', 'Count']
                    fig_tree = px.treemap(cat_counts, path=['Category'], values='Count', color='Count', color_continuous_scale='Reds')
                    st.plotly_chart(fig_tree, use_container_width=True)
                    st.dataframe(issue_df, use_container_width=True)

        # --- 模块 5: 词云辅助 ---
        st.markdown('<div class="section-title">👁️ 语义聚类词云</div>', unsafe_allow_html=True)
        wc_c1, wc_c2 = st.columns(2)
        with wc_c1:
            st.subheader("🔴 负面关键词")
            neg_text = " ".join(df[df['true_sentiment'] == 'Negative']['content'].tolist())
            if neg_text:
                wc_n = WordCloud(width=600, height=300, background_color='white', colormap='Reds').generate(re.sub(r'[^a-z\s]', '', neg_text.lower()))
                fig_n, ax_n = plt.subplots(); ax_n.imshow(wc_n); ax_n.axis('off'); st.pyplot(fig_n)
        with wc_c2:
            st.subheader("🟢 正面关键词")
            pos_text = " ".join(df[df['true_sentiment'] == 'Positive']['content'].tolist())
            if pos_text:
                wc_p = WordCloud(width=600, height=300, background_color='white', colormap='Greens').generate(re.sub(r'[^a-z\s]', '', pos_text.lower()))
                fig_p, ax_p = plt.subplots(); ax_p.imshow(wc_p); ax_p.axis('off'); st.pyplot(fig_p)

        # --- 模块 6: 原始数据筛选 ---
        st.markdown('<div class="section-title">🔍 原始数据下钻</div>', unsafe_allow_html=True)
        f1, f2, f3 = st.columns([1, 1, 1.5])
        with f1: s_filter = st.multiselect("星级:", [5,4,3,2,1], default=[1,2])
        with f2: v_filter = st.multiselect("版本:", df['reviewCreatedVersion'].dropna().unique().tolist())
        with f3: k_filter = st.text_input("搜索关键词:")
        
        filtered = df[df['score'].isin(s_filter)]
        if v_filter: filtered = filtered[filtered['reviewCreatedVersion'].isin(v_filter)]
        if k_filter: filtered = filtered[filtered['content'].str.contains(k_filter, case=False, na=False)]
        
        st.dataframe(filtered[['at', 'score', 'content', 'reviewCreatedVersion']].sort_values('at', ascending=False), 
                     use_container_width=True, height=400, hide_index=True,
                     column_config={"score": st.column_config.NumberColumn("评分", format="%d ⭐"), "content": "评论内容", "at": "时间"})
