...
# 【新增】基于规则的轻量级业务维度提取，用于快速生成图表，节省 Token
...
def get_structured_ai_report(model, df):
    ...
    你必须严格输出一个合法的 JSON 格式，不要包含任何 Markdown 标记符（如 ```json），直接输出 JSON 文本：
    {{
        "executive_summary": "用一段话概括当前版本的最致命危机（限100字）",
        "top_issues": [
            {{
                "issue_name": "痛点名称（如：广告频次过高）",
                "severity_score": 9,  // 1-10的严重程度评分
...
**优势 (优势分析)：**
1. **渲染效率大幅跃升**：摒弃了耗时的 `wordcloud` 和 `matplotlib`（往往需要阻塞几秒钟），全面拥抱 `plotly` 交互式图表。前端树状图（Treemap）和热力图支持悬浮查看具体数值，操作极其丝滑。
2. **AI 的“确定性”输出**：强制大模型将分析结果输出为 JSON。这样可以将评分转化为进度条 (`st.progress`)，将痛点转化为并排的业务卡片，这才是成熟 B 端 SaaS 产品的标准形态，而不是给业务方扔一段冰冷的 Markdown 聊天记录。
3. **架构解耦控制成本**：大盘数据分类由 Python 的轻量正则（`categorize_review`）完成；AI 仅在点击按钮时，对筛选出的“高质量差评”进行集中诊断。有效避免了 Token 浪费和超长等待。

**劣势 (风险与置信度评级)：**
* **置信度评级（中）：Python 正则提取的维度**。基于关键词库（如遇 ads 归为广告）的 `categorize_review` 属于硬编码方案。如果玩家使用了俚语或拼写错误，此方案无法像 AI 那样具备强悍的上下文理解能力，可能导致基础图表出现一定的数据逃逸。
* **规避建议**：如果在生产环境中资金与算力充裕，可将这一步替换为使用轻量级本地开源模型（如 Llama-3-8B 等）进行全量数据的预处理打标。对于目前的 Gemini API 额度，当前的混合式架构是最优解。
