...
<style>
    /* 使用 CSS 变量适配明暗主题 */
    .report-header { font-size: 2.2em; font-weight: 800; border-bottom: 2px solid var(--secondary-background-color); padding-bottom: 10px; margin-bottom: 20px;}
    .section-title { font-size: 1.4em; font-weight: 700; color: #1f77b4; margin-top: 30px; margin-bottom: 15px;}
    /* 优化指标卡片 */
...
# 基于规则的轻量级业务维度提取，用于快速生成图表，极大节省 Token
def categorize_review(text):
    text = str(text).lower()
    if re.search(r'ad|ads|commercial|advertisement', text): return '广告体验 (Ads)'
    if re.search(r'crash|freeze|screen|bug|stuck|load', text): return '系统稳定性 (Bugs)'
    if re.search(r'hard|difficult|impossible|level|pass', text): return '关卡难度 (Difficulty)'
    if re.search(r'money|pay|coin|expensive|cash', text): return '商业化/经济 (Monetization)'
    if re.search(r'fun|great|good|love|awesome', text): return '正面口碑 (Praise)'
    return '常规反馈 (Others)'
...
def load_and_clean_data(file_path):
    try:
        ...
        df['issue_category'] = df['content'].apply(categorize_review)
...
# 核心诊断函数：强制 LLM 输出结构化 JSON 数据，并使用正则安全提取
def get_structured_ai_report(model, df):
    # 针对负面和相关性高的评论进行抽样，提高诊断浓度
    target_df = df[(df['score'] <= 3) & (df['content'].str.len() > 15)]
    sample_reviews = target_df.sample(min(80, len(target_df)))['content'].tolist()
    text_to_analyze = "\n".join(sample_reviews)
    
    prompt = f"""
    作为传音高级游戏数据分析师，阅读以下真实玩家负面评论抽样。
    你必须严格输出一个合法的 JSON 格式，不要包含任何 Markdown 标记符，直接输出 JSON 文本：
    {{
        "executive_summary": "用一段话概括当前版本的最致命危机（限100字）",
        "top_issues": [
            {{
                "issue_name": "痛点名称（如：广告频次过高）",
                "severity_score": 9, 
                "player_voice": "提取一句最具代表性的玩家原话",
                "action_suggestion": "一句话改进建议"
            }}
        ],
        "strategic_advice": "针对策划或研发的中长期战略建议（限50字）"
    }}
    评论数据源: {text_to_analyze}
    """
    try:
        response = model.generate_content(prompt)
        raw_text = response.text.strip()
        
        # 正则表达式兜底抓取，无视外围可能的废话或 Markdown code blocks
        ...
        return {"error": f"AI 解析失败: {str(e)}", "raw_response": response.text if 'response' in locals() else ""}

# 【新增】缓存拦截器：Streamlit Cloud 专属的防 429 神器
@st.cache_data(show_spinner=False)
def generate_cached_report(_model, data_fingerprint, df):
    # _model 加下划线防哈希报错；data_fingerprint 用于判断源数据是否更新
    return get_structured_ai_report(_model, df)
...
if not os.path.exists(DATA_FILENAME):
    ...
    with st.spinner("正在加载并结构化业务数据..."):
        df = load_and_clean_data(DATA_FILENAME)
        
    if df is not None:
        st.markdown('<div class="report-header">📱 Bubble Shooter VOC 智能体检看板</div>', unsafe_allow_html=True)
        st.caption(f"分析样本区间: {df['date'].min()} 至 {df['date'].max()} | 数据源: Google Play")
        
        # --- 模块 1: 📊 实时大盘指标 ---
        st.markdown('<div class="section-title">📊 大盘健康度与基础指标</div>', unsafe_allow_html=True)
        col1, col2, col3, col4 = st.columns(4)
        col1.metric("总样本量", f"{len(df):,}")
        col2.metric("平均星级评分", f"{df['score'].mean():.2f} / 5.0")
        neg_pct = (len(df[df['true_sentiment'] == 'Negative']) / len(df)) * 100
        col3.metric("文本负面口碑率", f"{neg_pct:.1f}%", delta="预警线: 20%", delta_color="inverse")
        col4.metric("高频触燃业务线", df['issue_category'].mode()[0])

        # --- 模块 2: 📈 评分趋势与预警 ---
        st.markdown('<div class="section-title">📈 评分趋势与异常波动预警</div>', unsafe_allow_html=True)
        
        weekly_trend = df.groupby('week').agg(avg_score=('score', 'mean'), count=('score', 'count')).reset_index()
        weekly_trend['is_anomaly'] = weekly_trend['avg_score'] < 3.8 
        
        fig_trend = px.line(weekly_trend, x='week', y='avg_score', markers=True, title="自然周平均星级走势")
        anomalies = weekly_trend[weekly_trend['is_anomaly']]
        if not anomalies.empty:
            fig_trend.add_trace(go.Scatter(
                x=anomalies['week'], y=anomalies['avg_score'],
                mode='markers', marker=dict(color='red', size=12, symbol='x'),
                name='预警节点'
            ))
        fig_trend.update_layout(yaxis=dict(range=[1, 5]), height=350)
        st.plotly_chart(fig_trend, use_container_width=True)

        # --- 模块 3: 🗺️ 问题域归因拆解 (替代传统词云) ---
        st.markdown('<div class="section-title">🗺️ 问题域归因拆解</div>', unsafe_allow_html=True)
        c1, c2 = st.columns(2)
        
        with c1:
            st.caption("🗣️ 玩家讨论声量 Treemap (按业务维度)")
            tree_data = df.groupby(['issue_category', 'true_sentiment']).size().reset_index(name='count')
            fig_tree = px.treemap(tree_data, path=[px.Constant("全盘反馈"), 'issue_category', 'true_sentiment'], 
                                  values='count', color='true_sentiment',
                                  color_discrete_map={'Negative':'#ff9999', 'Neutral':'#c2c2c2', 'Positive':'#99ff99'})
            fig_tree.update_layout(margin=dict(t=10, l=10, r=10, b=10), height=400)
            st.plotly_chart(fig_tree, use_container_width=True)
            
        with c2:
            st.caption("🔥 版本与问题交叉热力图 (精准定位劣化版本)")
            heat_df = df[df['issue_category'] != '正面口碑 (Praise)']
            top_versions = heat_df['reviewCreatedVersion'].dropna().value_counts().nlargest(10).index
            heat_df = heat_df[heat_df['reviewCreatedVersion'].isin(top_versions)]
            fig_heat = px.density_heatmap(heat_df, x='reviewCreatedVersion', y='issue_category', 
                                          color_continuous_scale="Reds", text_auto=True)
            fig_heat.update_layout(xaxis_title="游戏版本", yaxis_title="客诉分类", height=400)
            st.plotly_chart(fig_heat, use_container_width=True)

        # --- 模块 4: 🤖 AI 高管诊断卡片 (组件化 + 全局缓存) ---
        st.markdown('<div class="section-title">🤖 AI 深度诊断报告</div>', unsafe_allow_html=True)
        
        # 定义数据指纹：行数 + 最后日期的组合。如果不变更源文件，指纹恒定，一直走缓存。
        current_data_fingerprint = f"{len(df)}_{df['date'].max()}"
        
        btn_col1, btn_col2 = st.columns([2, 8])
        with btn_col1:
            generate_clicked = st.button("⚡ 执行 AI 深度归因与策略推演", type="primary")
        with btn_col2:
            if st.button("🗑️ 强制清除云端缓存 (重新耗费 API)"):
                st.cache_data.clear()
                st.rerun()

        # 使用 session_state 控制本页面的显示状态
        ...
        if generate_clicked:
            if not YOUR_GEMINI_API_KEY:
                ...
                with st.spinner('🤖 AI 正在读取负面评论切片，提取高优 JSON 参数...'):
                    # 触发缓存函数
                    report_data = generate_cached_report(gemini_model, current_data_fingerprint, df)
                    
                    if "error" in report_data:
                        st.error("AI 结构化输出失败，请重试或检查提示词。")
                        with st.expander("查看原始输出"):
                            st.write(report_data.get("raw_response", ""))
                    else:
                        st.session_state.ai_report_display = report_data
                        st.success("✅ 报告就绪！（若数据无更新，其他用户访问将秒级读取云端缓存）")

        # 结果渲染 (脱离按钮判断，只要状态里有数据就显示)
        report_data = st.session_state.ai_report_display
        if report_data:
            st.markdown(f"""
            <div class="ai-summary-box">
                <strong>🎯 执行摘要：</strong><br>{report_data.get('executive_summary', '无数据')}
            </div>
            """, unsafe_allow_html=True)
            
            st.subheader("⚠️ 核心痛点与行动建议")
            issues = report_data.get('top_issues', [])
            
            cols = st.columns(len(issues) if issues else 1)
            for idx, issue in enumerate(issues[:3]): # 限制最多显示3个卡片
                with cols[idx]:
                    st.markdown(f"**{issue.get('issue_name', '未知问题')}**")
                    severity = issue.get('severity_score', 5)
                    st.progress(severity / 10.0, text=f"严重级别: {severity}/10")
                    st.info(f"💬 原话: *\"{issue.get('player_voice', '')}\"*")
                    st.success(f"🛠️ 建议: {issue.get('action_suggestion', '')}")
            
            st.markdown("---")
            st.markdown(f"**🧭 长期战略指引:** {report_data.get('strategic_advice', '无数据')}")
        ...
        with filter_c2:
            version_filter = st.multiselect("📱 筛选应用版本", options=df['reviewCreatedVersion'].dropna().unique().tolist(), default=[])
        with filter_c3:
            search_kw = st.text_input("🔑 关键词检索", "")
